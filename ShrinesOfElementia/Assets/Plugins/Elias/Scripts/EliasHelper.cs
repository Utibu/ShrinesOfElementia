...
public class EliasHelper
{
	...
	/// </summary>
	public EliasHelper(string file) : this(Path.Combine(Application.streamingAssetsPath, file), null, GetPath(Path.Combine(Application.streamingAssetsPath, file)), (uint)AudioSettings.outputSampleRate, 2, 2048, null, 128, 4096)
	{
	}
	...
	/// </summary>
	public EliasHelper(string file, int channels, int framesPerBuffer, int cachePageCount, int cachePageSize) : 
        this(Path.Combine(Application.streamingAssetsPath, file), null, GetPath(Path.Combine(Application.streamingAssetsPath, file)), (uint)AudioSettings.outputSampleRate, (byte)channels, (ushort)framesPerBuffer, null, cachePageCount, cachePageSize)
	{
	}

	public EliasHelper(string file, EliasWrapper.elias_data_reader_functions? readerFunctions, string basePath, uint sampleRate, byte channels, ushort framesPerBuffer, EliasWrapper.elias_memory_functions? memoryFunctions, int cachePageCount, int cachePageSize)
	{
	...
	private void Initialize(EliasWrapper.elias_data_reader_functions? readerFunctions, string basePath, uint sampleRate, byte channels, ushort framesPerBuffer, EliasWrapper.elias_memory_functions? memoryFunctions, int cachePageCount, int cachePageSize)
    {
